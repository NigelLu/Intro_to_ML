...
def robustPCA(X, delta=1e-6, mu=None, maxiter=1):
	"""
	...
	while i < max(maxiter, 1):
		...
		u, sigma_arrary, vt = np.linalg.svd(step1_temp)
		...
		for i in range(len(truncated_sigma)):
			truncated_sigma_matrix[i,i] = truncated_sigma[i]
